        ...
        for idx, row in df.iterrows():
            ...
            records.append(
                ElectricVehicleRecord(
                    ...
                    vin_1_to_10=row.get("VIN (1-10)", "").strip() or None,
                    county=row.get("County", "").strip() or None,
                    city=row.get("City", "").strip() or None,
                    state=row.get("State", "").strip() or "未知",  # 确保非空（避免空字符串）
                    model_year=model_year,
                    make=row.get("Make", "").strip() or None,
                    model=row.get("Model", "").strip() or None,
                    ev_type=row.get("Electric Vehicle Type", "").strip() or None,
                    cafv_eligibility=row.get("Clean Alternative Fuel Vehicle (CAFV) Eligibility", "").strip() or None,
                    electric_range=electric_range,
                    base_msrp=base_msrp,
                    electric_utility=row.get("Electric Utility", "").strip() or None,
...
if __name__ == "__main__":
    ...
    # 示例2：统计华盛顿州的电动汽车总数
    wa_count = EVDataQuery.get_state_ev_count("Washington")
